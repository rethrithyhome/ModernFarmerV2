...
| GET | `/api/catalog/suppliers/by-phone?phone=...` | ពិនិត្យស្ទួនភ្លាមៗ — ត្រឡប់ `null` បើគ្មាន |
...
| GET | `/api/reports/revenue-trend?days=14` (និន្នាការ ៧–៩០ ថ្ងៃ បំពេញសូន្យ) |
...
### ដំណាក់កាល ៨ — ក្រាហ្វិកលើ Dashboard
- ✅ SVG ដោយផ្ទាល់ គ្មាន library — ធ្វើឱ្យ bundle តូច សមស្របនឹង PWA
- ✅ `/revenue-trend` បំពេញថ្ងៃគ្មានលក់ដោយ ០ — ២ សប្តាហ៍ថ្មីៗឃើញ ៥ ថ្ងៃដំបូងគ្មានលក់ ៣០ ថ្ងៃចុងក្រោយពិត
- ✅ តេស្តព្រំដែន (server-render): ចំណុចតែមួយ · គ្រប់តម្លៃជា ០ — មិនកើត `NaN` ក្នុងកូអរដោនេ SVG
- ✅ `days` clamp ចន្លោះ ៧–៩០ ទោះស្នើ ៥០០
...
### ដំណាក់កាល ៩ — ដោះស្រាយបញ្ហាគាំង + ស្វែងរកតាមលេខទូរស័ព្ទ

**១. «ម៉ឺនុយហិរញ្ញវត្ថុគាំង» — រកឃើញ root cause ពិត តាមរយៈតេស្ត React ពិត**

មូលហេតុ: `useData(path, { enabled })` ពេល `enabled` ប្តូរ false→true (ប្តូរផ្ទាំង)
React commit ការប្តូរ state មុន រួចទើប run effect ក្រោយ។ មានរយៈពេលខ្លីមួយដែល
`loading` នៅជា `false` ចាស់ ខណៈ `data` នៅជា `null` — ទំព័រ Finance.jsx សរសេរ
`x.loading ? Loading : x.data.length` ដោយគ្មានការពារ ធ្វើឱ្យអាន `null.length`
ហើយ React គាំង (unmount ដោយគ្មានការជូនដំណឹង)។

- ✅ សរសេរតេស្តត្រាប់តាមការចុចប្តូរផ្ទាំងជាក់ស្តែង (jsdom + React ពិត + act()) —
  **តេស្តនេះបង្កើតកំហុសដដែលបេះបិទ** ជាមួយកូដចាស់ (`Cannot read properties of null`)
- ✅ កែនៅកម្រិត `useData` hook ខ្លួនឯង (មិនកែតែ Finance.jsx) — ធានាកុំឱ្យ
  `data === null && loading === false` កើតឡើងព្រមគ្នាពេល `enabled=true`
- ✅ ក្រោយកែ តេស្តដដែលជាប់ — LOADING បង្ហាញត្រឹមត្រូវ រហូតទិន្នន័យមកដល់
- ✅ ទិន្នន័យក្នុងឃ្លាំង (cache) បង្ហាញភ្លាមៗ គ្មាន LOADING ស្ទួន

**២. ស្វែងរកអតិថិជនតាមលេខទូរស័ព្ទ — រកឃើញកំហុសខាង backend**

មូលហេតុ: ការស្វែងរកចាស់ប្រៀបធៀបនឹង `phone_e164` (`+85596777888`) ដែល**គ្មានលេខ
0 ដើម** (ជំនួសដោយកូដប្រទេស)។ អ្នកប្រើកម្ពុជាវាយលេខតាមទម្រង់ក្នុងស្រុក
(`096777888`, មាន 0 ដើម) ជានិច្ច — ការវាយបែបនេះ **គ្មានថ្ងៃណាចូលគ្នាបានទេ**
ព្រោះ `0` មិនមានក្នុង `+855...`។

- ✅ តេស្តជាមួយទិន្នន័យពិត៖ វាយ `096777888` លើកមុន → **0 លទ្ធផល** (បញ្ហា) →
  ក្រោយកែ → **១ លទ្ធផលត្រឹមត្រូវ**
- ✅ កែឱ្យប្រៀបធៀបនឹង `phone_display` ជំនួស (រក្សា 0 ដើម) ដោយបំបាត់ចន្លោះ/សញ្ញា
  មុនប្រៀបធៀប (`regexp_replace`) ដូច្នេះ "096 777 888" និង "096777888" ផ្គូផ្គងគ្នា
- ✅ ការពារកុំឱ្យផ្គូផ្គងលើសកម្រិត៖ លេខតិចជាង ៣ ខ្ទង់ → មិនធ្វើសកម្មភាពស្វែងរកលេខ
  (បើគ្មានការពារនេះ ការស្វែងរកតាមឈ្មោះសុទ្ធនឹងយកគ្រប់គ្នាមកខុសឆ្គង ព្រោះ pattern
  ទទេ `LIKE '%%'` ផ្គូផ្គងគ្រប់ជួរ)
- ✅ ស្វែងរកដោយផ្នែក (`777888`) ដំណើរការត្រឹមត្រូវ ស្វែងរកឈ្មោះសុទ្ធនៅត្រឹមត្រូវដដែល

**៣. SearchSelect — ប្រអប់ជ្រើសរើសដោយវាយស្វែងរក** (ជំនួសបញ្ជីទម្លាក់ចុះ)

ប្រើក្នុង «លក់ថ្មី» (ស្វែងរកអតិថិជនតាមម៉ាស៊ីនមេ ពន្យារ ៣០០ms) និង
«បញ្ជាទិញថ្មី» (ស្វែងរកអ្នកផ្គត់ផ្គង់ក្នុងម៉ាស៊ីនភ្ជាប់ ព្រោះបញ្ជីតូច)។
ដោះស្រាយបញ្ហាឈ្មោះ/លេខទូរស័ព្ទដូចគ្នាច្រឡំគ្នា ដោយបង្ហាញលេខទូរស័ព្ទរួមជាមួយឈ្មោះ។

- ✅ វាយឈ្មោះដូចគ្នា ២ នាក់ → ឃើញលេខទូរស័ព្ទដើម្បីញែក
- ✅ វាយលេខទូរស័ព្ទដោយគ្មានចន្លោះ ("096777888") ផ្គូផ្គងនឹងលេខបង្ហាញមានចន្លោះ ("096 777 888")
- ✅ ជ្រើសរើស/សម្អាត/ចុចខាងក្រៅបិទបញ្ជី — ដំណើរការត្រឹមត្រូវទាំងអស់ (៦ តេស្ត)

### ដំណាក់កាល ១០ — លេខទូរស័ព្ទអ្នកផ្គត់ផ្គង់មិនស្ទួនគ្នា

**Database constraint** (`migration_006`) — `UNIQUE INDEX` លើ `phone_e164`
ការពារកម្រិត database ដែលជាកម្រិតសុវត្ថិភាពចុងក្រោយ ទោះកូដខាងលើមានចន្លោះខ្វះខាត។

Migration សរសេរឱ្យសុវត្ថិភាពចំពោះទិន្នន័យចាស់ — បើមានលេខស្ទួនរួចហើយ (ដែលអាចមាន
ព្រោះប្រព័ន្ធពីមុនអនុញ្ញាត) migration **មិនគាំងទេ** គ្រាន់តែរំលង ហើយប្រាប់ query
សម្រាប់រកអ្នកផ្គត់ផ្គង់ស្ទួនដើម្បីសម្រួចដោយដៃមុន។

- ✅ តេស្តលើ database ស្អាត → constraint បង្កើតបានធម្មតា
- ✅ តេស្តលើ database ដែលមានលេខស្ទួនចេតនា → migration **មិនគាំង** បង្ហាញការណែនាំ
- ✅ ក្រោយសម្រួចទិន្នន័យស្ទួន រត់ម្តងទៀត → constraint បង្កើតបានជោគជ័យ

**ការពារកម្រិត UI** — «បន្ថែមអ្នកផ្គត់ផ្គង់» ដាក់លេខទូរស័ព្ទជាជួរដំបូង។
ក្រោយឈប់វាយ ៤០០ms ប្រព័ន្ធពិនិត្យស្វ័យប្រវត្តិ (endpoint `/suppliers/by-phone`)
ដោយមិនចាំបាច់រង់ចាំដល់ចុច «រក្សាទុក» ទើបដឹងថាស្ទួន។

- ✅ លេខមានស្រាប់ → ព្រមានភ្លាមៗ «មានកត់ត្រារួចហើយ — ឈ្មោះ "..."» + បិទប៊ូតុងរក្សាទុក
- ✅ លេខថ្មី → គ្មានការព្រមាន ប៊ូតុងបើកធម្មតា
- ✅ លេខតិចជាង ៨ ខ្ទង់ → មិនហៅម៉ាស៊ីនមេ (កុំខ្ជះខ្ជាយសំណើពេលកំពុងវាយ)
- ✅ ការទប់ស្កាត់ក្រោយសរសេរ (backend `409`) បង្ហាញឈ្មោះម្ចាស់លេខផងដែរ — ការពារទោះ
  ជាមធ្យោបាយផ្សេងហៅ API ដោយផ្ទាល់ (ឧ. ខួបគ្នា ចុចរក្សាទុកមុនការពិនិត្យស្វ័យប្រវត្តិចប់)
- ✅ កែលេខទូរស័ព្ទទៅជាលេខអ្នកផ្គត់ផ្គង់ផ្សេង → បដិសេធ
- ✅ កែឈ្មោះខ្លួនឯងឱ្យប្រើលេខទូរស័ព្ទខ្លួនឯងដដែល → មិនចាត់ទុកជាស្ទួន (ធ្វើការធម្មតា)
...
└── src/
    ...
    ├── Charts.jsx   # ក្រាហ្វិក SVG សាមញ្ញ (និន្នាការ · ជញ្ជាំង)
    ├── QueueBanner.jsx
