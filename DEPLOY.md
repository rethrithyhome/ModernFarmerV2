...
4. ធ្វើដដែលសម្រាប់ `db/migration_003_offline.sql` · `db/migration_004_users.sql` · `db/migration_005_currency_riel.sql` · `db/migration_006_supplier_phone_unique.sql`
...
រត់តាមលំដាប់ខាងលើ (schema → 002 → 003 → 004 → 005 → 006 → seed) ព្រោះនីមួយៗពឹងផ្អែកលើមុន។ បើឃើញ "Success. No rows returned" គឺត្រឹមត្រូវហើយ។
