...
import { Button, Field, Panel, Table, Text } from "./kit";
import { CloudSettings } from "./CloudPanel";
import { NavIcon, type IconId } from "./icons";

type PanelKey = "identity" | "cloud" | "user" | "data" | "guide";

const TILES: { key: PanelKey; icon: IconId; label: string; hint: string }[] = [
  { key: "identity", icon: "business", label: "អត្តសញ្ញាណអ្នកលក់", hint: "ព័ត៌មានលើវិក្កយបត្រ" },
  { key: "cloud", icon: "cloud", label: "Cloud Sync", hint: "ភ្ជាប់ Supabase" },
  { key: "user", icon: "user", label: "អ្នកប្រើប្រាស់", hint: "ឈ្មោះអ្នកកត់ត្រា" },
  { key: "data", icon: "database", label: "ទិន្នន័យ", hint: "ទាញយក · បញ្ចូល · សម្អាត" },
  { key: "guide", icon: "guide", label: "របៀបប្រើ", hint: "លំដាប់ការងារប្រចាំថ្ងៃ" },
];
...
export function Settings({ db }: { db: DbShape }) {
  ...
  if (panel === null) {
    return (
      <div className="settings-grid">
        {TILES.map((t) => (
          <button key={t.key} type="button" className="settings-tile" onClick={() => setPanel(t.key)}>
            <NavIcon id={t.icon} size={26} />
            <span className="settings-tile-label">{t.label}</span>
            <span className="settings-tile-hint">{t.hint}</span>
          </button>
        ))}
      </div>
    );
  ...
  return (
    <div className="stack">
      ...
      >
        <NavIcon id="back" size={16} /> ត្រឡប់ទៅការកំណត់
      </button>
      ...
      {panel === "user" && (
        <Panel title="អ្នកប្រើប្រាស់ និងឧបករណ៍" hint="ឈ្មោះនេះនឹងត្រូវកត់ជា «អ្នកទទួលខុសត្រូវ» ពេលកត់ស្តុក ឬបើក Lot">
          ...
          <p className="panel-hint" style={{ marginTop: "0.8rem" }}>
            ...
            ឬកុំព្យូទ័រមួយទៀត៖ ចុច «ទាញយក JSON» នៅផ្ទាំង ទិន្នន័យ រួច «បញ្ចូល JSON» លើឧបករណ៍ថ្មី។
          </p>
        </Panel>
      )}
      ...
      {panel === "data" && (
        <>
          ...
          <Panel title="បញ្ចូលទិន្នន័យ" hint="ការបញ្ចូលនឹងលុបទិន្នន័យបច្ចុប្បន្ន ហើយជំនួសដោយឯកសារ JSON">
            ...
            <div className="row-actions">
              <Button variant="primary" disabled={!json.trim()} onClick={() => setMessage(importJson(json))}>
                បញ្ចូលពីប្រអប់អក្សរ
              </Button>
            </div>
            {message && (
